...
async def safe_reply(update: Update, text: str, reply_markup=None):
    ...
    Handles 'message is not modified' quietly.
    Splits long messages when needed.
    """
    cq = getattr(update, "callback_query", None)
    if cq is not None:
        try:
            await cq.edit_message_text(text[:4096], reply_markup=reply_markup)
            return
...
async def edit_or_send(update: Update, text: str, reply_markup=None):
    """
    Try editing; if that fails or nothing changed, send as new message.
    """
    cq = getattr(update, "callback_query", None)
    if cq is not None:
        try:
...
def fetch_collection_token_ids_live(collection_id: str, page_cap: int = 20000) -> list[str]:
    """Fetch all tokenIds from Enjin live (no cache)."""
    q = """
...
# ─────────────────────────────────────────────
# SQLite (shared for prizes + dice + collection & wallet cache)
DICE_DB = "ect.db"
...
def init_db():
    ...
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_ctok_col ON collection_tokens(collection_id);")
    ...
    CREATE TABLE IF NOT EXISTS wallet_tokens (
        ...
        PRIMARY KEY (address, collection_id, token_id)
    );
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_wtok_addr ON wallet_tokens(address);")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_wtok_addr_col ON wallet_tokens(address, collection_id);")
...
def cache_get_tokens(collection_id: str) -> list[str]:
    conn = db_conn(); cur = conn.cursor()
    cur.execute("SELECT token_id FROM collection_tokens WHERE collection_id=? ORDER BY token_id", (collection_id,))
    rows = [r[0] for r in cur.fetchall()]
    conn.close()
    return rows

def cache_set_tokens(collection_id: str, token_ids: list[str]):
    conn = db_conn(); cur = conn.cursor()
    cur.execute("INSERT INTO collections(collection_id, total_items, last_updated) VALUES(?,?,?) "
                "ON CONFLICT(collection_id) DO UPDATE SET total_items=excluded.total_items, last_updated=excluded.last_updated",
                (collection_id, len(token_ids), int(time.time())))
    cur.executemany("INSERT OR IGNORE INTO collection_tokens(collection_id, token_id) VALUES(?,?)",
                    [(collection_id, tid) for tid in token_ids])
    conn.commit(); conn.close()

def cache_add_tokens(collection_id: str, new_ids: list[str]):
    if not new_ids:
        return 0
    conn = db_conn(); cur = conn.cursor()
    cur.executemany("INSERT OR IGNORE INTO collection_tokens(collection_id, token_id) VALUES(?,?)",
                    [(collection_id, tid) for tid in new_ids])
    cur.execute("SELECT COUNT(*) FROM collection_tokens WHERE collection_id=?", (collection_id,))
    total = int(cur.fetchone()[0])
    cur.execute("INSERT INTO collections(collection_id, total_items, last_updated) VALUES(?,?,?) "
                "ON CONFLICT(collection_id) DO UPDATE SET total_items=?, last_updated=?",
                (collection_id, total, int(time.time()), total, int(time.time())))
    conn.commit(); conn.close()
    return len(new_ids)

# ── Wallet cache helpers
def wallet_cache_get_all(address: str) -> list[tuple[str, str]]:
    conn = db_conn(); cur = conn.cursor()
    cur.execute("SELECT collection_id, token_id FROM wallet_tokens WHERE address=?", (address,))
    rows = [(r[0], r[1]) for r in cur.fetchall()]
    conn.close()
    return rows

def wallet_cache_set_snapshot(address: str, items: list[tuple[str, str, int, int]]):
    now = int(time.time())
    conn = db_conn(); cur = conn.cursor()
    cur.execute("DELETE FROM wallet_tokens WHERE address=?", (address,))
    cur.executemany(
        "INSERT OR REPLACE INTO wallet_tokens(address, collection_id, token_id, balance, reserved) VALUES(?,?,?,?,?)",
        [(address, cid, tid, bal, rsv) for (cid, tid, bal, rsv) in items]
    )
    cur.execute(
        "INSERT INTO wallet_snapshots(address, last_updated) VALUES(?, ?) "
        "ON CONFLICT(address) DO UPDATE SET last_updated=excluded.last_updated",
        (address, now)
    )
    conn.commit(); conn.close()

def wallet_cache_last_updated(address: str) -> int | None:
    conn = db_conn(); cur = conn.cursor()
    cur.execute("SELECT last_updated FROM wallet_snapshots WHERE address=?", (address,))
    row = cur.fetchone()
    conn.close()
    return int(row[0]) if row else None

# Smart get: fast from cache, background refresh if needed
async def get_collection_token_ids_smart(update: Update, context: ContextTypes.DEFAULT_TYPE, collection_id: str) -> list[str]:
    cached = cache_get_tokens(collection_id)
    asyncio.create_task(_background_refresh_collection(update, context, collection_id))
    return cached

async def _background_refresh_collection(update: Update, context: ContextTypes.DEFAULT_TYPE, collection_id: str):
    try:
        live_ids = fetch_collection_token_ids_live(collection_id)
    except Exception:
        return
    live_set = set(live_ids)
    cached_set = set(cache_get_tokens(collection_id))
    new_ids = sorted(list(live_set - cached_set), key=lambda s: (0, int(s)) if s.isdigit() else (1, s))
    if new_ids:
        added = cache_add_tokens(collection_id, new_ids)
        uid = update.effective_user.id if getattr(update, "effective_user", None) else None
        if uid is not None:
            prog = context.user_data.get("progress")
            if prog and prog.get("cid") == collection_id:
                all_ids = sort_token_ids(cache_get_tokens(collection_id))
                prog["ids"] = all_ids
                context.user_data["progress"] = prog
                try:
                    await safe_reply(update, f"✅ Collection updated: +{added} new tokens found.")
                except Exception:
                    pass
                try:
        ...
        cache_set_tokens(collection_id, live_ids)

# ── Smart wallet fetch (cache + auto-refresh)
async def get_wallet_owned_by_collection_smart(update: Update, context: ContextTypes.DEFAULT_TYPE, address: str) -> dict[str, set[str]]:
    cached_pairs = wallet_cache_get_all(address)
    owned: dict[str, set[str]] = defaultdict(set)
    for cid, tid in cached_pairs:
        owned[cid].add(tid)
    asyncio.create_task(_background_refresh_wallet(update, context, address))
    return owned

async def _background_refresh_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE, address: str, force: bool = False):
    # throttle to avoid too frequent scans (2 min)
    last = wallet_cache_last_updated(address)
    if (not force) and last and (int(time.time()) - last < 120):
        return
    ...
    for e in edges:
        ...
        if (bal + rsv) > 0:
            ...
            items.append((cid, tid, bal, rsv))

    before = set(wallet_cache_get_all(address))
    after = set((cid, tid) for (cid, tid, _, _) in items)
    if after != before:
        wallet_cache_set_snapshot(address, items)
        uid = update.effective_user.id if getattr(update, "effective_user", None) else None
        if uid is not None:
            prog = context.user_data.get("progress")
            if prog:
                cid = prog.get("cid")
                if cid:
                    latest = wallet_cache_get_all(address)
                    have = {tid for (cc, tid) in latest if cc == cid}
                    prog["have"] = set(have)
                    context.user_data["progress"] = prog
                    try:
                        await safe_reply(update, "✅ Wallet updated — refreshing your progress.")
                    except Exception:
                        pass
                    try:
                        await render_progress_page(update, context, edit=True)
...
async def render_progress_page(update: Update, context: ContextTypes.DEFAULT_TYPE, edit: bool = False):
    ...
    prog_copy = {**s, "have": sorted(list(s.get("have", set())))}  # JSON-safe
...
async def mycollections(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FAST: build from wallet cache
    owned_map = await get_wallet_owned_by_collection_smart(update, context, addr)
    if not owned_map:
        # if cache empty, background refresh will fill soon; inform user
        await update.message.reply_text("Fetching your wallet… one sec. Try again in a moment.")
        return

    counts = {cid: len(tset) for cid, tset in owned_map.items()}
...
async def collections(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Use cached collection token IDs if present (then background refresh)
    cached_ids = cache_get_tokens(cid)
    if not cached_ids:
        try:
            live_ids = fetch_collection_token_ids_live(cid)
        ...
        if not live_ids:
            ...
            return
        cache_set_tokens(cid, live_ids)
        cached_ids = live_ids

    sorted_ids = sort_token_ids(cached_ids)
    # FAST: owned tokens from wallet cache
    owned_map = await get_wallet_owned_by_collection_smart(update, context, addr)
    have_set = owned_map.get(cid, set())
    ...
    asyncio.create_task(_background_refresh_collection(update, context, cid))
    asyncio.create_task(_background_refresh_wallet(update, context, addr))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Selecting from Find results (set collection) — jump into Progress (cached + background refresh)
...
# ─────────────────────────────────────────────
# 🎲 Dice Game — 2d6 with 4s cooldown
ROLL_COOLDOWN = 4
...
async def dice_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "dice:wait":      return
    if data == "dice:close":     await q.edit_message_text("🎲 Dice closed."); return

# ─────────────────────────────────────────────
# App wiring
def main():
    init_db()  # ensure ect.db & tables exist
